class SPRestApi {
        ...
        async addItem(data = {}, lista = this.listaAtual) {
            ...
            const payload = JSON.stringify({
                "__metadata": { "type": this.encodeEntityType(lista, lista === this.listaAtual) },
                "Title": "",
                ...data
            });
        ...
        async addAttachment(itemId, fileName, fileContent, lista = this.listaAtual) {
            const formDigest = await this.getFormDigestValue();
            const url = this.buildListUrl(lista, `/items(${itemId})/AttachmentFiles/add(FileName='${fileName}',overwrite=true)`);
            const headers = {
                "Accept": "application/json;odata=verbose",
                "X-RequestDigest": formDigest,
        ...
        async upsertItem(data = {}, lista = this.listaAtual) {
            if(data.Id) return this.updateItem(data.Id, Object.keys(data).filter(k=>k!=='Id').reduce((obj, k)=>({ ...obj, [k]: data[k]}), {}), lista);
            return this.addItem(data, lista);
        }
